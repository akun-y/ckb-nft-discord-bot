...
export async function coldUpdateRolesForUser(
  client: Client,
  userID: string,
  ...
  const coldUpdateDiscordRolesForUserInGuild = async (
    ...
    const coldUpdateDiscordRolesForUserInGuildRule = async (
      ...
      let numMatchingTokens: number = 0
      if (isNFTRule(rule)) {
    ...
    await (guildConfigDoc.data() as GuildConfig).rules.reduce(
      (p, guildRule) => {
        return p.then(() => coldUpdateDiscordRolesForUserInGuildRule(guildRule))
      },
  ...
  await guildConfigsSnapshot.docs.reduce((p, guildConfigDoc) => {
    ...
    return p.then(async () => {
      try {
        ...
        console.log('update user role err...', e)
      }
    })
